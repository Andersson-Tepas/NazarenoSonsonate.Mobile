    ...
    public class RecorridoService
    {
        ...
        private async Task PreloadInternoAsync()
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine($"Error API en preload: {ex.Message}");
        ...
        public async Task<List<RecorridoDto>> ObtenerRecorridosAsync(bool forzarRecarga = false, bool permitirApi = true)
        {
            ...
            List<RecorridoDto> result;

            try
            {
                result = await _httpClient.GetFromJsonAsync<List<RecorridoDto>>("api/recorridos")
                         ?? new List<RecorridoDto>();
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error API al obtener recorridos: {ex.Message}");
                result = new List<RecorridoDto>();
            }
        ...
        public async Task<RecorridoDto?> ObtenerRecorridoPorIdAsync(int id, bool forzarRecarga = false, bool permitirApi = true)
        {
            ...
            {
                Console.WriteLine($"Error API al obtener recorrido por id {id}: {ex.Message}");
                return null;
            }
        }
